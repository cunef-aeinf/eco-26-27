...
Now create a new variable called `compensationUSD` that converts the original `CompensationAmount` to USD. This is done by multiplying  `CompensationAmount` by `exchangeRate`. Save the new dataset under the name `datascience`.
...
Create a new dataset called `compensation_summary` containing the median compensation in USD for each `Major`.
...
```{r}
ggplot(compensation_summary, aes(x=fct_reorder(Major,medianCompensation), y=medianCompensation)) + geom_bar(stat="identity") + labs(title="Median salary of data scientists", subtitle="2017 Kaggle Survey", x="", y="Salary (USD)") + coord_flip() + theme_minimal()
```
...
The variables in this dataset are `GEOID`, `NAME`, `estimate_income`, `estimate_rent`, `moe_income`, `moe_rent`.
...
Extract the airports from the `airports` dataset for which there is some flight whose destination is that airport in the `flights` dataset.
...
(Hard)
Draw a map of the United States where each destination airport appears as a point.
...
flights %>% filter( !(dest %in% c("ANC", "HNL")) ) %>% left_join(airports, by=c("dest"="faa")) %>% 
  group_by(dest) %>% summarise(med_air_time = mean(air_time, na.rm=T), lat=lat, lon=lon) %>% ggplot(aes(lon, lat)) +
    borders("state") +
    geom_point(aes(size=med_air_time)) + theme_classic() +
    coord_quickmap()
```
...
flights %>% filter( !(dest %in% c("ANC", "HNL")) ) %>% 
  ...
  mutate(date = ifelse(month == 5, "May 13", "June 13")) %>% 
  left_join(airports, by=c("dest"="faa")) %>% 
  group_by(dest, date) %>% summarise(med_delay = mean(dep_delay, na.rm=T), lat=lat, lon=lon) %>% ggplot(aes(lon, lat)) +
    borders("state") +
    geom_point(aes(size=med_delay)) + theme_classic() +
    coord_quickmap() + facet_grid(date~.)
```
